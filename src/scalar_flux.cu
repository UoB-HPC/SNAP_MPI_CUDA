    ...
    )
{
    ...
    dim3 blocks(problem->ng, ceil(rankinfo->nx*rankinfo->ny*rankinfo->nz/(float)RED_BLOCK_SIZE), 1);
    dim3 threads(power, RED_BLOCK_SIZE, 1);

    cudaEventRecord(events->scalar_flux_event_start);
    check_cuda("Recording scalar flux start event");

    reduce_flux<<< blocks, threads, sizeof(double)*power*RED_BLOCK_SIZE >>>(
    ...
    )
{
    ...
    reduce_flux_moments<<< blocks, threads, sizeof(double)*power*RED_BLOCK_SIZE >>>(
        rankinfo->nx, rankinfo->ny, rankinfo->nz,
